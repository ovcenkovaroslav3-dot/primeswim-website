...
/*
  Видео есть только там, где техника прошла покадровую проверку. Этот список
  намеренно короче списка стилей: баттерфляй остаётся статичным, пока не будет
  корректной петли. Для каждого принятого ролика браузеру нужны оба формата.
*/
test('у проверенных анимаций гребка есть WebM и MP4', () => {
  for (const id of ['freestyle', 'backstroke', 'breaststroke']) {
    for (const ext of ['webm', 'mp4']) {
      assert.ok(
        existsSync(join(DIR, `${id}.${ext}`)),
        `${id}: нет ${id}.${ext} — пересоберите node scripts/make-stroke-motion.mjs`,
      );
    }
  }
});
