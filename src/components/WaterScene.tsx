...
void main() {
  ...
  // диапазон сужен: провал в тень внизу резче, верх светлее — та самая
  // штормовая вертикаль, из-за которой кадр читается драматично, а не ровно
  float depth = smoothstep(-0.42, 0.88, p.y);
  vec3 col = mix(u_deep, u_mid, depth);

  // ── два источника света в толще
  col += u_violet * 1.34 * exp(-length((p - vec2(0.62 + u_mouse.x * 0.06, -0.55)) * vec2(0.75, 1.0)) * 1.5);
...
export function WaterScene({
  /*
    Палитра сцены подтянута к штормовому референсу: провал в тень глубже,
    средний тон насыщеннее, а свет уведён из бледно-сиреневого в бледную
    бирюзу — так светится вода, подсвеченная снизу.

    Бирюза намеренно почти обесцвечена. Полноценный аква был убран из
    палитры сайта как третий конкурирующий акцент, и возвращать его нельзя.
    Здесь это не акцент интерфейса, а цвет света в толще: он не спорит ни
    с фиолетовым, ни с лаймом, потому что не встречается ни на одной кнопке.
  */
  deep = '#07000e',
  mid = '#26073f',
  violet = '#8a28cd',
