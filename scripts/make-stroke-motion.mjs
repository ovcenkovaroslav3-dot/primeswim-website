...
for (const file of files.sort()) {
  ...
  */
  /*
    ...
    продолжить то, что рядом. Продолжать нужно СНИЗУ, со стороны воды, и это
    вторая правка по тому же месту.

    Сначала полоса делилась пополам: верх брал строку сверху, низ — снизу.
    Логично для однородного участка и неверно там, где по линии проходит край
    тела, — а у баттерфляя он идёт по ней всю длину. Верхняя половина полосы
    брала фон и прорезала тело щелью. Замер по среднему яркости ряда её не
    ловил: щель видна только над телом, а это меньше половины ширины.

    Снизу же под линией у всех четырёх стилей вода и корпус пловца. Где по обе
    стороны одно и то же, ничего не меняется; где край — тело подрастает вверх
    на толщину полосы, то есть на три-четыре пикселя из восьмисот. Этого не
    видно, а щели не возникает никогда.
  ...
  */
  ...
  const copyFrom = (ch) =>
    `if(between(Y,${lineTop},${lineBottom}),${ch}(X,${lineBottom + 1}),${ch}(X,Y))`;
  const erase =
    line.share > 0.5
