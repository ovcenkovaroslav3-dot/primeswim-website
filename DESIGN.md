...
Знак — **не персонаж**. Мультяшный талисман с первого экрана снят 13 сентября
2026: он называл возраст («кружок для малышей») там, где школа готовит
разряды. Полный разбор, геометрия, места применения и промты для генераций —
в [`docs/brand-prime-orca.md`](docs/brand-prime-orca.md). Реализация —
`src/components/OrcaMark.tsx`.

Короткое правило: **как фон — не выше 8 % прозрачности, как элемент — не
чаще одного раза на экран, никогда с мимикой и аксессуарами.**
