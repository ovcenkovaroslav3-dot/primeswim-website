...
У WHOOP от нуля на body до -0.04em на display. У нас `-0.02em` стоит и на h1,
и на обоих размерах `SectionHeading` — приём перенесён полностью. (Запись
«на крупных h2 трекинг нулевой» устарела: проверено 13 сентября 2026, там
`tracking-[-0.02em]`.)

**Вес заголовков секций — 400, и это последняя вычищенная запись от Aqua
Voice.** 19 сентября 2026: все h2 и h1 подстраниц стояли в `font-extralight`,
то есть весом 200 — тем самым, который был центральным тезисом прежнего
ориентира («авторитет берётся сдержанностью, а не весом»). Половину тезиса
владелец отклонил ещё 10 сентября, оставив h1 в Unbounded extrabold, но до
заголовков секций правка тогда не дошла. Выходил раскол: заголовок первого
экрана весом 800 кричит, следом девять заголовков весом 200 шепчут, между
ними пусто.

У WHOOP display набран весом 400 при body 400 — иерархия строится размером,
а не тонкостью. К этому и приведено. Цена: страница выросла на 47 px из 9210,
на две строки разъехался один заголовок из восьми.

**Волосяные числа остаются.** «13 лет», «850 ₽», «45 мин», «до 12», телефон
в «Контактах» — это другой приём: крупная тонкая цифра рядом с плотной
подписью читается как величина, а не как заголовок. С весом 400 у h2 они не
конкурируют, а до правки сливались.
...
Второе правило — **«не центровать абзацы, текст выравнивается влево при
max-width 520px»** — проверено и отклонено для секции записи (`FinalCta`).
Собрали левый вариант и сравнили снимки: заголовок и лид встают на одну
левую линию с карточками мессенджеров и формой, но правая половина шапки
остаётся пустой, а тело секции при этом идёт во всю ширину. Левый
заголовок над симметричным телом требует второй колонки справа — картинки
или блока текста, — и её здесь нет и брать неоткуда.

К тому же запрет у WHOOP написан про body copy внутри карточек. У нас
центрованы только надзаголовок, заголовок в три строки и лид: это шапка
секции, а не проза. Сама форма и карточки мессенджеров и так выровнены
влево (`text-left` на обёртке `LeadForm`).

Центровка остаётся. Секция — последняя на странице и единственная
центрованная: ось схождения перед формой работает ровно там, где нужно
принять решение. Повторно не открывать.
...
## СОБСТВЕННЫЕ ПРАВИЛА, КОТОРЫХ НЕТ У РЕФЕРЕНСА

Три вещи, которых в WHOOP нет вовсе, а у нас они есть и обязательны.

### Прайми и её знак

У WHOOP знак — надпись. У нас к надписи добавлена косатка: «PRIMESWIM»
ничем не отличается от полусотни таких же надписей в выдаче, а силуэт
узнаётся раньше чтения.

**Прайми** — талисман школы, косатка. Живёт в двух воплощениях, и это не
дубликаты:

- **объёмный рендер** — сам персонаж, стоит на сайте ровно один раз, на
  первом экране (`src/components/MascotOrca.tsx`);
- **плоский вектор** — тот же силуэт в шапке, подвале, меню, фавиконе и
  фоновыми присутствиями в тёмных секциях (`src/components/OrcaMark.tsx`).

**Рендер для присутствия, вектор для знаков.** На 20 пикселях рендер
превращается в пятно, а вектор читается; на первом экране в натуральную
величину вектор выглядел бы наклейкой там, где нужен персонаж.

Мультяшный талисман — улыбающаяся косатка в шапочке — снят 13 сентября 2026:
он называл возраст («кружок для малышей») там, где школа готовит разряды.
Полный разбор, геометрия, места применения и промты — в
[`docs/brand-prime-orca.md`](docs/brand-prime-orca.md).

Короткое правило: **как фон — не выше 8 % прозрачности и подбирается по
снимку для каждой секции заново; как элемент — не чаще одного раза на экран;
никогда с мимикой и аксессуарами.**

### Зона нажатия — 44 px

Ни одно самостоятельное действие на сайте не бывает ниже 44 px. Ссылки
«читать дальше» набирают этот рост через `ArrowLink` (`src/components/ui.tsx`),
кнопки — через размеры в `buttonClass`.

Появилось после замера: семь ссылок вида «Всё расписание →» стояли ростом
28 px — вдвое меньше подушечки пальца. Отрицательный внешний отступ
возвращает добавленную высоту обратно в вёрстку, поэтому ритм абзацев
от этого правила не меняется.

Исключение одно и оно в стандарте: ссылка внутри строки текста
(«напишите нам в Telegram») высоту строки не набирает и не должна.

**Правило про палец, а не про курсор.** Замер 18 сентября 2026 показал, что
одним `ArrowLink` дело не кончилось: по 28 px стояли одиннадцать ссылок
подвала, телефон в подвале, ссылка на Яндекс Карты, телефон в мобильном меню,
а телефон в «Контактах» — 32. Всё поднято до 44. Но в подвале ссылки идут
подряд, отрицательный отступ там наложил бы зоны друг на друга, и настоящие
44 px разъезжали подвал на 176 px — на телефоне это ничего не стоит, на
десктопе столбец разделов превращал два соседних в пустоту.

Поэтому там, **и только там**, под мышью шаг возвращается к 28 через
`pointer-fine`. Это не послабление: 44 px в этом файле объяснены подушечкой
пальца, у мыши её нет, а 28 выше минимума WCAG 2.5.8 (24 px). Заводить
`pointer-fine` где-то ещё без такого же замера — нельзя.

**Три зоны ниже 44 остаются намеренно, и каждая объяснена на месте:** полоса
каналов над шапкой — 36 px (`SocialLinks`, `variant="bar"`), хлебные крошки —
32 px (`Breadcrumbs`), обе ради того, чтобы полоса и цепочка остались тонкими.
Обе выше минимума WCAG. Четвёртой не заводить.

### Вертикальный ритм секций — 112 px, и три исключения

Все секции сайта дышат одинаково: `py-14 sm:py-16 md:py-28`, то есть 56 px на
телефоне и 112 px на десктопе. Так задано в `Section` (`src/components/ui.tsx`),
и секции со своей разметкой обязаны повторять эту же тройку.

Исключений ровно три, и каждое названо:

| Секция | Отступ | Почему |
|---|---|---|
| `Hero` | свой | первый экран держит собственную композицию |
| `ProofStrip` | 40 px | это полоса, а не блок: заголовка нет, каждый пункт сам себе утверждение |
| `FinalCta` | 144 px | закрывающая полоса, ей нужен воздух перед решением |

Проверено замером 13 сентября 2026: `CoachPreview` и `PoolPreview` стояли на
`md:py-24` — на 16 px ниже всех остальных, без единой причины. Приведены к
системе. Четвёртого исключения не заводить: строка «96 против 112» в макете
не читается как решение, она читается как невычитанный код.

### Фотография — только своя площадка

У WHOOP фотография постановочная, но своя. У нас правило жёстче:
**кадр, снятый не в МГИК, не может стоять там, где обещает место.**

Из этого следует и запрет на сгенерированную воду, бассейн и пловцов на
сайте — как бы хорошо они ни выглядели. Это не вопрос вкуса: сгенерированный
бассейн на первом экране — обещание не того места. Генерации отведены
Reels, мессенджеры и мерч (см. `docs/brand-prime-orca.md`, раздел
